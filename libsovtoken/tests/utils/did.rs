...
pub fn create_nym(
    ...
    let (did, verkey) = indy::did::create_and_store_my_did(wallet_handle, "{}").unwrap();
    ...
    indy::ledger::sign_and_submit_request(pool_handle, wallet_handle, &did_trustee, &req_nym)?;
...
pub fn initial_trustee(wallet_handle: i32) -> DidAndVerKey {
    ...
    indy::did::create_and_store_my_did(wallet_handle, &first_json_seed).unwrap()
}
