...
pub fn send_get_utxo_request(wallet: &Wallet, pool_handle: i32, did: &str, address: &str) -> Vec<UTXO> {
    let (req, method) = indy::payments::build_get_payment_sources_request(wallet.handle, Some(did), address).unwrap();
    let res = indy::ledger::sign_and_submit_request(pool_handle, wallet.handle, did, &req).unwrap();
    let parsed_resp = indy::payments::parse_get_payment_sources_response(&method, &res).unwrap();
