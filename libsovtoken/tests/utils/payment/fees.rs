...
pub fn set_fees(pool_handle: i32, wallet_handle: i32, payment_method: &str, fees: &str, dids: &Vec<&str>, submitter_did: Option<&str>) -> String {
    let set_fees_req = indy::payments::build_set_txn_fees_req(wallet_handle, submitter_did, payment_method, &fees).unwrap();

    let set_fees_req = Request::<SetFees>::multi_sign_request(wallet_handle, &set_fees_req, dids.to_vec()).unwrap();

    indy::ledger::submit_request(pool_handle, &set_fees_req).unwrap()
}
...
pub fn get_fees(wallet: &Wallet, pool_handle: i32, submitter_did: Option<&str>) -> String {
    ...
    let result = indy::ledger::submit_request(pool_handle, &get_fees_req).unwrap();
    indy::payments::parse_get_txn_fees_response(PAYMENT_METHOD_NAME, &result).unwrap()
}