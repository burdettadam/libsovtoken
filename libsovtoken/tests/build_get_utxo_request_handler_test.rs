...
pub fn build_and_submit_get_utxo_request() {
    ...
    let (get_utxo_req, payment_method) = indy::payments::build_get_payment_sources_request(wallet.handle, Some(dids[0]), &payment_addresses[0]).unwrap();
    let res = indy::ledger::sign_and_submit_request(pool_handle, wallet.handle, dids[0], &get_utxo_req).unwrap();
    let res = indy::payments::parse_get_payment_sources_response(&payment_method, &res).unwrap();
...
pub fn payment_address_is_identifier() {
    ...
    let (get_utxo_req, _) = indy::payments::build_get_payment_sources_request(wallet.handle, Some(dids[0]), &payment_addresses[0]).unwrap();
