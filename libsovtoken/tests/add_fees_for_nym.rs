...
pub fn build_and_submit_nym_with_fees() {
    ...
    let (did_new, verkey_new) = indy::did::create_and_store_my_did(wallet.handle, "{}").unwrap();

    let nym_req = indy::ledger::build_nym_request(dids[0], &did_new,  Some(&verkey_new), None, None).unwrap();
    let nym_req_signed = indy::ledger::sign_request(wallet.handle, dids[0], &nym_req).unwrap();
    let (nym_req_with_fees, pm) = indy::payments::add_request_fees(wallet.handle, Some(dids[0]), &nym_req_signed, &inputs, &outputs, None).unwrap();
    let nym_resp = indy::ledger::submit_request(pool_handle, &nym_req_with_fees).unwrap();
    let parsed_resp = indy::payments::parse_response_with_fees(&pm, &nym_resp).unwrap();
    ...
    let get_nym_req = indy::ledger::build_get_nym_request(Some(dids[0]), &did_new).unwrap();
    let get_nym_resp = indy::ledger::sign_and_submit_request(pool_handle, wallet.handle, dids[0], &get_nym_req).unwrap();
...
pub fn build_and_submit_nym_with_fees_insufficient_funds() {
    ...
    let (nym_req_with_fees, pm) = indy::payments::add_request_fees(wallet.handle, Some(dids[0]), &nym_req_signed, &inputs, &outputs, None).unwrap();
    let nym_resp = indy::ledger::submit_request(pool_handle, &nym_req_with_fees).unwrap();
    let err = indy::payments::parse_response_with_fees(&pm, &nym_resp).unwrap_err();
...
pub fn build_and_submit_nym_with_no_fees() {
    ...
    let nym_req = indy::ledger::build_nym_request(dids[0], &did_new,  Some(&verkey_new), None, None).unwrap();
    let nym_req_signed = indy::ledger::sign_request(wallet.handle, dids[0], &nym_req).unwrap();
    let nym_resp = indy::ledger::submit_request(pool_handle, &nym_req_signed).unwrap();
    let resp = indy::payments::parse_response_with_fees("sov", &nym_resp).unwrap();
...
pub fn build_and_submit_nym_with_fees_utxo_already_spent() {
    ...
    let (nym_req_with_fees, pm) = indy::payments::add_request_fees(wallet.handle, Some(dids[0]), &nym_req_signed, &inputs, &outputs, None).unwrap();
    let nym_resp = indy::ledger::submit_request(pool_handle, &nym_req_with_fees).unwrap();
    indy::payments::parse_response_with_fees(&pm, &nym_resp).unwrap();

    let (did_new_2, verkey_new_2) = indy::did::create_and_store_my_did(wallet.handle, "{}").unwrap();

    let nym_req = indy::ledger::build_nym_request(dids[0], &did_new_2,  Some(&verkey_new_2), None, None).unwrap();
    let nym_req_signed = indy::ledger::sign_request(wallet.handle, dids[0], &nym_req).unwrap();
    let (nym_req_with_fees, pm) = indy::payments::add_request_fees(wallet.handle, Some(dids[0]), &nym_req_signed, &inputs, &outputs, None).unwrap();
    let nym_resp = indy::ledger::submit_request(pool_handle, &nym_req_with_fees).unwrap();
    let err = indy::payments::parse_response_with_fees(&pm, &nym_resp).unwrap_err();
...
pub fn build_and_submit_nym_with_fees_from_invalid_did_and_check_utxo_remain_unspent() {
    ...
    let (did_new, verkey_new) = indy::did::create_and_store_my_did(wallet.handle, "{}").unwrap();
    let (did_new_2, _) = indy::did::create_and_store_my_did(wallet.handle, "{}").unwrap();

    let nym_req = indy::ledger::build_nym_request(&did_new_2, &did_new,  Some(&verkey_new), None, None).unwrap();
    let (nym_req_with_fees, pm) = indy::payments::add_request_fees(wallet.handle, Some(dids[0]), &nym_req, &inputs, &outputs, None).unwrap();
    let resp = indy::ledger::sign_and_submit_request(pool_handle, wallet.handle, dids[0], &nym_req_with_fees).unwrap();
    let err = indy::payments::parse_response_with_fees(&pm, &resp).unwrap_err();
...
pub fn build_and_submit_nym_with_fees_from_other_nym_txn() {
    ...
    let (did_new_1, verkey_new_1) = indy::did::create_and_store_my_did(wallet.handle, "{}").unwrap();
    let (did_new_2, verkey_new_2) = indy::did::create_and_store_my_did(wallet.handle, "{}").unwrap();

    let nym_req_1 = indy::ledger::build_nym_request(dids[0], &did_new_1,  Some(&verkey_new_1), None, None).unwrap();
    let nym_req_2 = indy::ledger::build_nym_request(dids[1], &did_new_2,  Some(&verkey_new_2), None, None).unwrap();

    let (nym_req_with_fees_1, pm) = indy::payments::add_request_fees(wallet.handle, Some(dids[0]), &nym_req_1, &inputs, &outputs, None).unwrap();
    ...
    let resp = indy::ledger::sign_and_submit_request(pool_handle, wallet.handle, dids[1], &nym_req_with_fees_2).unwrap();
    let err = indy::payments::parse_response_with_fees(&pm, &resp).unwrap_err();
    assert_eq!(err, ErrorCode::CommonInvalidStructure);

    let resp = indy::ledger::sign_and_submit_request(pool_handle, wallet.handle, dids[0], &nym_req_with_fees_1).unwrap();
    indy::payments::parse_response_with_fees(&pm, &resp).unwrap();
}
